...
@implementation UserRemoveRelation

- (UserRemoveRelation *)initRequestWithDic:(NSDictionary *)dic
{
    if((self = (UserRemoveRelation *)[super initWithBackendManagerName:@"uim"]))
    {
        self.paramDic = dic;
    }
    return self;
...
- (void)requestDidFinished:(ASIHTTPRequest *)theRequest
{
    ...
    if(responseDic != nil)
    {
        ...
        if([errorCodeString isEqualToString:@"0000"])
        {
            ...
            if(self.delegate && [self.delegate respondsToSelector:@selector(userRemoveRelationDidFinishedWithParamters:)])
            {
                objc_msgSend(self.delegate, @selector(userRemoveRelationDidFinishedWithParamters:),outputDic);
            }
        }
        else
        {
            ...
            {
                objc_msgSend(self.delegate, @selector(userRemoveRelationDidFinishedWithErrorMessage:),errorCodeString);
            }
        }
    }
    else
    {
        if(self.delegate && [self.delegate respondsToSelector:@selector(userRemoveRelationDidFinishedWithErrorMessage:)])
        {
            objc_msgSend(self.delegate, @selector(userRemoveRelationDidFinishedWithErrorMessage:),AppLocalizedString(@"服务出错,稍后再试..."));
        }
    }
