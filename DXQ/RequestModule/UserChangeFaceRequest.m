...
@implementation UserChangeFaceRequest
@synthesize paramDic = _paramDic;
@synthesize delegate = _delegate;

- (UserChangeFaceRequest *)initRequestWithDic:(NSDictionary *)dic
{
    if((self = (UserChangeFaceRequest *)[super initWithBackendManagerName:@"uim"]))
    {
        self.paramDic = dic;
    }
    return self;
...
- (void)requestDidFinished:(ASIHTTPRequest *)theRequest
{
    ...
    if(responseDic != nil)
    {
        ...
        if([errorCodeString isEqualToString:@"0000"])
        {
            ...
            if(self.delegate && [self.delegate respondsToSelector:@selector(userChangeFaceRequestDidFinishedWithParamters:)])
            {
                objc_msgSend(self.delegate, @selector(userChangeFaceRequestDidFinishedWithParamters:),outputDic);
            }
        }
        else
        {
            if(self.delegate && [self.delegate respondsToSelector:@selector(userChangeFaceRequestDidFinishedWithErrorMessage:)])
            {
                objc_msgSend(self.delegate, @selector(userChangeFaceRequestDidFinishedWithErrorMessage:),errorCodeString);
            }
        }
    }
    else
    {
        if(self.delegate && [self.delegate respondsToSelector:@selector(userChangeFaceRequestDidFinishedWithErrorMessage:)])
        {
            objc_msgSend(self.delegate, @selector(userChangeFaceRequestDidFinishedWithErrorMessage:),AppLocalizedString(@"服务出错,稍后再试..."));
        }
    }
}
...
- (void)requestDidFailed:(ASIHTTPRequest *)theRequest
{
    if(self.delegate && [self.delegate respondsToSelector:@selector(userChangeFaceRequestDidFinishedWithErrorMessage:)])
    {
        objc_msgSend(self.delegate, @selector(userChangeFaceRequestDidFinishedWithErrorMessage:),AppLocalizedString(@"网络错误"));
    }
}
