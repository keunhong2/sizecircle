...
#import "FriendsCircleRequest.h"

@implementation FriendsCircleRequest
...
- (void)requestDidFinished:(ASIHTTPRequest *)theRequest
{
    ...
    if(responseDic != nil)
    {
        ...
        if([errorCodeString isEqualToString:@"0000"])
        {
            ...
            if(self.delegate && [self.delegate respondsToSelector:@selector(friendsCircleRequestRequest:didFinishWithData:)])
            {
                objc_msgSend(self.delegate, @selector(friendsCircleRequestRequest:didFinishWithData:),self,outputDic);
            }
        }
        else
        {
            if(self.delegate && [self.delegate respondsToSelector:@selector(friendsCircleRequestRequest:didFailedWithErrorMsg:)])
            {
                objc_msgSend(self.delegate, @selector(friendsCircleRequestRequest:didFailedWithErrorMsg:),self,errorCodeString);
            }
        }
    }
    else
    {
        if(self.delegate && [self.delegate respondsToSelector:@selector(friendsCircleRequestRequest:didFailedWithErrorMsg:)])
        {
            objc_msgSend(self.delegate, @selector(friendsCircleRequestRequest:didFailedWithErrorMsg:),self,AppLocalizedString(@"服务出错,稍后再试..."));
        }
    }
}
...
- (void)requestDidFailed:(ASIHTTPRequest *)theRequest
{
    if(self.delegate && [self.delegate respondsToSelector:@selector(friendsCircleRequestRequest:didFailedWithErrorMsg:)])
    {
        objc_msgSend(self.delegate, @selector(friendsCircleRequestRequest:didFailedWithErrorMsg:),self,AppLocalizedString(@"网络错误"));
    }
}
