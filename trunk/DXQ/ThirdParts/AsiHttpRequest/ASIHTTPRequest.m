...
- (void)cancelOnRequestThread
{
	...
	HYLog(@"[STATUS] Request cancelled: %@",self);
	#endif
...
- (void)startSynchronous
{
	...
	HYLog(@"[STATUS] Starting synchronous request %@",self);
#endif
...
{
#if DEBUG_REQUEST_STATUS || DEBUG_THROTTLING
	HYLog(@"[STATUS] Starting asynchronous request %@",self);
#endif
...
- (void)applyAuthorizationHeader
{
	...
	if (![self shouldPresentCredentialsBeforeChallenge]) {
		...
		HYLog(@"[AUTH] Request %@ will not send credentials to the server until it asks for them",self);
		#endif
		return;
	}
	...
	if (![[self requestHeaders] objectForKey:@"Authorization"]) {
		...
		if ([self username] && [self password] && [[self authenticationScheme] isEqualToString:(NSString *)kCFHTTPAuthenticationSchemeBasic]) {
			...
			#if DEBUG_HTTP_AUTHENTICATION
			HYLog(@"[AUTH] Request %@ has a username and password set, and was manually configured to use BASIC. Will send credentials without waiting for an authentication challenge",self);	
			#endif
				...
				if (credentials) {
					...
					if ([credentials objectForKey:@"Authentication"]) {
						...
						if (CFHTTPMessageApplyCredentialDictionary(request, (CFHTTPAuthenticationRef)[credentials objectForKey:@"Authentication"], (CFDictionaryRef)[credentials objectForKey:@"Credentials"], NULL)) {
							...
							#if DEBUG_HTTP_AUTHENTICATION
							HYLog(@"[AUTH] Request %@ found cached credentials (%@), will reuse without waiting for an authentication challenge",self,[credentials objectForKey:@"AuthenticationScheme"]);
							#endif
						} else {
							[[self class] removeAuthenticationCredentialsFromSessionStore:[credentials objectForKey:@"Credentials"]];
							#if DEBUG_HTTP_AUTHENTICATION
							HYLog(@"[AUTH] Failed to apply cached credentials to request %@. These will be removed from the session store, and this request will wait for an authentication challenge",self);
							#endif
						}
					...
					// When this happens, we'll need to create the Authorization header ourselves
						...
						#if DEBUG_HTTP_AUTHENTICATION
						HYLog(@"[AUTH] Request %@ found cached BASIC credentials from a previous request. Will send credentials without waiting for an authentication challenge",self);
						#endif
					}
				}
			}
	...
	if ([self shouldAttemptPersistentConnection]) {
		...
		if ([self connectionInfo]) {
			...
			// Check if we should have expired this connection
				...
				HYLog(@"[CONNECTION] Not re-using connection #%i because it has expired",[[[self connectionInfo] objectForKey:@"id"] intValue]);
				#endif
				...
				#if DEBUG_PERSISTENT_CONNECTIONS
                HYLog(@"%@ - Not re-using connection #%i for request #%i because it is already used by request #%i",self,[[[self connectionInfo] objectForKey:@"id"] intValue],[[self requestID] intValue],[[[self connectionInfo] objectForKey:@"request"] intValue]);
				#endif
                [self setConnectionInfo:nil];
            }
		}
		...
		#if DEBUG_PERSISTENT_CONNECTIONS
		HYLog(@"[CONNECTION] Request #%@ will use connection #%i",[self requestID],[[[self connectionInfo] objectForKey:@"id"] intValue]);
		#endif
		...
		#if DEBUG_PERSISTENT_CONNECTIONS
		HYLog(@"[CONNECTION] Request %@ will not use a persistent connection",self);
		#endif
	}
		...
		if ([self postLength]) {
			...
			if (totalBytesSent > lastBytesSent) {
				...
				if ([self totalBytesSent] == [self postLength]) {
					HYLog(@"[STATUS] Request %@ finished uploading data",self);
				}
				#endif
			}
		}
...
{
#if DEBUG_REQUEST_STATUS || DEBUG_THROTTLING
	HYLog(@"[STATUS] Request finished: %@",self);
#endif
	if ([self error] || [self mainRequest]) {
		return;
	}
...
- (void)readResponseHeaders
{
	...
	#if DEBUG_REQUEST_STATUS
	if ([self totalBytesSent] == [self postLength]) {
		HYLog(@"[STATUS] Request %@ received response headers",self);
	}
	#endif		
	...
	if ([self responseStatusCode] == 401) {
		...
		if ([self authenticationScheme]) {
			HYLog(@"[AUTH] Request %@ has passed %@ authentication",self,[self authenticationScheme]);
		}
		#endif
	}
	...
	if (![self authenticationNeeded]) {
		...
		if (!requestAuthentication && [[self authenticationScheme] isEqualToString:(NSString *)kCFHTTPAuthenticationSchemeBasic] && [self username] && [self password] && [self useSessionPersistence]) {
			...
			#if DEBUG_HTTP_AUTHENTICATION
			HYLog(@"[AUTH] Request %@ passed BASIC authentication, and will save credentials in the session store for future use",self);
			#endif
			...
			if (![connectionHeader isEqualToString:@"close"]) {
				...
				if (keepAliveHeader) { 
					...
					if (max > 5) {
						...
						#if DEBUG_PERSISTENT_CONNECTIONS
							HYLog(@"[CONNECTION] Got a keep-alive header, will keep this connection open for %f seconds", [self persistentConnectionTimeoutSeconds]);
						#endif					
					}
				...
				// Otherwise, we'll assume we can keep this connection open
					...
					[self setConnectionCanBeReused:YES];
					#if DEBUG_PERSISTENT_CONNECTIONS
						HYLog(@"[CONNECTION] Got no keep-alive header, will keep this connection open for %f seconds", [self persistentConnectionTimeoutSeconds]);
					#endif
				}
			}
		}
...
- (BOOL)willRedirect
{
	...
	#if DEBUG_REQUEST_STATUS
	HYLog(@"[STATUS] Request will redirect (code: %i): %@",responseCode,self);
	#endif
...
- (NSMutableDictionary *)findCredentials
{
	...
	if (user && pass) {
		...
		#if DEBUG_HTTP_AUTHENTICATION
		HYLog(@"[AUTH] Request %@ will use credentials set on its url",self);
		#endif
		...
		if ([self mainRequest] && [[self mainRequest] username] && [[self mainRequest] password]) {
			...
			#if DEBUG_HTTP_AUTHENTICATION
			HYLog(@"[AUTH] Request %@ will use credentials from its parent request",self);
			#endif
		...
		// Let's try to use the ones set in this object
			...
			#if DEBUG_HTTP_AUTHENTICATION
			HYLog(@"[AUTH] Request %@ will use username and password properties as credentials",self);
			#endif
		}		
	}
	...
	if ((!user || !pass) && useKeychainPersistence) {
		...
		if (authenticationCredentials) {
			...
			#if DEBUG_HTTP_AUTHENTICATION
			if (user && pass) {
				HYLog(@"[AUTH] Request %@ will use credentials from the keychain",self);
			}
			#endif
		}
	}
...
{
	#if DEBUG_HTTP_AUTHENTICATION
		HYLog(@"[AUTH] Request %@ received credentials from its delegate or an ASIAuthenticationDialog, will retry",self);
	#endif
...
{
	#if DEBUG_HTTP_AUTHENTICATION
		HYLog(@"[AUTH] Request %@ had authentication cancelled by its delegate or an ASIAuthenticationDialog",self);
	#endif
...
- (void)attemptToApplyCredentialsAndResume
{
	...
	if (!requestAuthentication) {
		#if DEBUG_HTTP_AUTHENTICATION
		HYLog(@"[AUTH] Request %@ failed to read authentication information from response headers",self);
		#endif
	...
	if (!CFHTTPAuthenticationRequiresAccountDomain(requestAuthentication)) {
		...
		if ([self authenticationScheme] != (NSString *)kCFHTTPAuthenticationSchemeNTLM || [self authenticationRetryCount] == 0) {
			HYLog(@"[AUTH] Request %@ received 401 challenge and must authenticate using %@%@",self,[self authenticationScheme],realm);
		} else {
			HYLog(@"[AUTH] Request %@ NTLM handshake step %i",self,[self authenticationRetryCount]+1);
		}
	#endif
	...
	if (!CFHTTPAuthenticationIsValid(requestAuthentication, &err)) {
		...
		if (err.domain == kCFStreamErrorDomainHTTP && (err.error == kCFStreamErrorHTTPAuthenticationBadUserName || err.error == kCFStreamErrorHTTPAuthenticationBadPassword)) {
			...
			#if DEBUG_HTTP_AUTHENTICATION
			HYLog(@"[AUTH] Request %@ had bad credentials, will remove them from the session store if they are cached",self);
			#endif
			...
			if ([self error] || [self isCancelled]) {
				...
				#if DEBUG_HTTP_AUTHENTICATION
				HYLog(@"[AUTH] Request %@ failed or was cancelled while waiting to access credentials",self);
				#endif
			...
			if ([self useSessionPersistence]) {
				...
				if (credentials && [self applyCredentials:[credentials objectForKey:@"Credentials"]]) {
					...
					#if DEBUG_HTTP_AUTHENTICATION
					HYLog(@"[AUTH] Request %@ will reuse cached credentials from the session (%@)",self,[credentials objectForKey:@"AuthenticationScheme"]);
					#endif
			...
			if ([self willAskDelegateForCredentials]) {
				...
				#if DEBUG_HTTP_AUTHENTICATION
				HYLog(@"[AUTH] Request %@ will ask its delegate for credentials to use",self);
				#endif
			...
			if ([self showAuthenticationDialog]) {
				...
				#if DEBUG_HTTP_AUTHENTICATION
				HYLog(@"[AUTH] Request %@ will ask ASIAuthenticationDialog for credentials",self);
				#endif
		...
		#if DEBUG_HTTP_AUTHENTICATION
		HYLog(@"[AUTH] Request %@ has no credentials to present and must give up",self);
		#endif
	...
	if (requestCredentials) {
		...
		if ((([self authenticationScheme] != (NSString *)kCFHTTPAuthenticationSchemeNTLM) || [self authenticationRetryCount] < 2) && [self applyCredentials:requestCredentials]) {
			...
			#if DEBUG_HTTP_AUTHENTICATION
			HYLog(@"[AUTH] Request %@ has failed NTLM authentication",self);
			#endif
			...
			#if DEBUG_HTTP_AUTHENTICATION
			HYLog(@"[AUTH] Request %@ had credentials and they were not marked as bad, but we got a 401 all the same.",self);
			#endif
		...
		if (newCredentials) {
			...
			if ([self applyCredentials:newCredentials]) {
				...
				#if DEBUG_HTTP_AUTHENTICATION
				HYLog(@"[AUTH] Request %@ failed to apply credentials",self);
				#endif
				[delegateAuthenticationLock unlock];
				[self failWithError:[NSError errorWithDomain:NetworkRequestErrorDomain code:ASIInternalErrorWhileApplyingCredentialsType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"Failed to apply credentials to request",NSLocalizedDescriptionKey,nil]]];
			}
...
{	
...
#if DEBUG_REQUEST_STATUS
	HYLog(@"[STATUS] Request %@ finished downloading data (%qu bytes)",self, [self totalBytesRead]);
#endif
	[self setStatusTimer:nil];
	...
	#if DEBUG_PERSISTENT_CONNECTIONS
	if ([self requestID]) {
		HYLog(@"[CONNECTION] Request #%@ finished using connection #%@",[self requestID], [[self connectionInfo] objectForKey:@"id"]);
	}
	#endif
	[[self connectionInfo] removeObjectForKey:@"request"];
	[[self connectionInfo] setObject:[NSDate dateWithTimeIntervalSinceNow:[self persistentConnectionTimeoutSeconds]] forKey:@"expires"];
...
- (BOOL)retryUsingNewConnection
{
	if ([self retryCount] == 0) {
		...
		#if DEBUG_PERSISTENT_CONNECTIONS
			HYLog(@"[CONNECTION] Request attempted to use connection #%@, but it has been closed - will retry with a new connection", [[self connectionInfo] objectForKey:@"id"]);
		#endif
		[connectionsLock lock];
		[[self connectionInfo] removeObjectForKey:@"request"];
		[persistentConnectionsPool removeObject:[self connectionInfo]];
	...
	#if DEBUG_PERSISTENT_CONNECTIONS
		HYLog(@"[CONNECTION] Request attempted to use connection #%@, but it has been closed - we have already retried with a new connection, so we must give up", [[self connectionInfo] objectForKey:@"id"]);
	#endif	
	return NO;
}
...
#if DEBUG_PERSISTENT_CONNECTIONS
			HYLog(@"[CONNECTION] Closing connection #%i because it has expired",[[existingConnection objectForKey:@"id"] intValue]);
#endif
		...
		if (throttleWakeUpTime) {
			if ([throttleWakeUpTime timeIntervalSinceDate:[NSDate date]] > 0) {
				if ([self readStreamIsScheduled]) {
					[self unscheduleReadStream];
					#if DEBUG_THROTTLING
					HYLog(@"[THROTTLING] Sleeping request %@ until after %@",self,throttleWakeUpTime);
					#endif
				}
			} else {
				if (![self readStreamIsScheduled]) {
					[self scheduleReadStream];
					#if DEBUG_THROTTLING
					HYLog(@"[THROTTLING] Waking up request %@",self);
					#endif
				}
			}
		} 
...
+ (void)recordBandwidthUsage
{
	...
	#if DEBUG_THROTTLING
	HYLog(@"[THROTTLING] ===Used: %u bytes of bandwidth in last measurement period===",bandwidthUsedInLastSecond);
	#endif
